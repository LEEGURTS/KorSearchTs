...
exports.choSearch = exports.engToKor = exports.korToEng = void 0;
const korDict = {
    ㄱ: "r",
    ㄲ: "R",
    ㄴ: "s",
...
const korToEng = (kor) => {
    ...
    const engArr = korArr.map((korChar) => {
        ...
        return (_a = korDict[korChar]) !== null && _a !== void 0 ? _a : korChar;
    });
    return engArr.join("");
};
    ...
    switch (stack.length) {
        case 5:
            ...
            if (choDict[stack[0]] &&
                ...
                const cho = choDict[stack[0]], jung = jungDict[stack[1] + stack[2]], jong = jongDict[stack[3] + stack[4]];
                result = codeToKor(CHO_DATA.indexOf(cho), JUNG_DATA.indexOf(jung), JONG_DATA.indexOf(jong) + 1);
                stack.length = 0;
            }
            break;
        case 4:
            ...
            if (choDict[stack[0]] &&
                ...
                jongDict[stack[3]]) {
                const cho = choDict[stack[0]], jung = jungDict[stack[1] + stack[2]], jong = jongDict[stack[3]];
                result = codeToKor(CHO_DATA.indexOf(cho), JUNG_DATA.indexOf(jung), JONG_DATA.indexOf(jong) + 1);
                stack.length = 0;
            }
            // 초 중 종종
            if (choDict[stack[0]] &&
                ...
                const cho = choDict[stack[0]], jung = jungDict[stack[1]], jong = jongDict[stack[2] + stack[3]];
                result = codeToKor(CHO_DATA.indexOf(cho), JUNG_DATA.indexOf(jung), JONG_DATA.indexOf(jong) + 1);
                stack.length = 0;
            }
            break;
        case 3:
            ...
            if (choDict[stack[0]] && jungDict[stack[1]] && jongDict[stack[2]]) {
                const cho = choDict[stack[0]], jung = jungDict[stack[1]], jong = jongDict[stack[2]];
                result = codeToKor(CHO_DATA.indexOf(cho), JUNG_DATA.indexOf(jung), JONG_DATA.indexOf(jong) + 1);
                stack.length = 0;
            }
            // 초 중중
            if (choDict[stack[0]] && jungDict[stack[1] + stack[2]]) {
                const cho = choDict[stack[0]], jung = jungDict[stack[1] + stack[2]];
        ...
        case 2:
            ...
            if (choDict[stack[0]] && jungDict[stack[1]]) {
                const cho = choDict[stack[0]], jung = jungDict[stack[1]];
                result = codeToKor(CHO_DATA.indexOf(cho), JUNG_DATA.indexOf(jung), 0);
                stack.length = 0;
            }
            // 중
...
const choCompare = (cho, target) => {
    // 초중종, 그 외 케이스
    if (cho === target)
        return true;
    if (!/^[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]$/.test(target))
        return cho === target;
    // 입력이 초성이라면
